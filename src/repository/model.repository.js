...
export function createModel(name, description, picture, pictureUserPet, user) {
  db.query(
    `
        ...
        VALUES ($1, $2, $3, $4, $5);`,
    [name, description, picture, pictureUserPet, user.id],
  );
}
...
export function getModel(id) {
  return db.query(
    `SELECT models.*, users.name AS "userName", users.cpf, users.picture AS "userPicture", users.telephone, users.email, users.description AS "userDescription"
    FROM models
